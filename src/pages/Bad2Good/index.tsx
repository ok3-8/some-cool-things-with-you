...
function Bad2Good() {
	...
	useEffect(() => {
		const getInfo: any[] = JSON.parse(
			window.localStorage.getItem("bad2good") || "[]",
		);
		const currentInfo = codeJson.ocean[currentIndex];
		if (
			!getInfo.some(
				(item: any) => item.id === currentInfo.language + currentInfo.id,
			)
		) {
			const o = {
				...
				title:
					...
					1 +
					"、" +
					currentInfo.description.substring(
						0,
						currentInfo.description.indexOf("\n"),
					),
	...
	return (
		<div className="App bad2good" id="Bad2Good">
			...
			<p className="text">
				Every day is a new start. Forget the bad and move on to the good. Be who
				you want and you will be great.
			</p>
